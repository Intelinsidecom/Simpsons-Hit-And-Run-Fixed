
// Automatically generated by Jeeves, don't edit.  Edit tlRoadSegmentChunk.sc instead

#ifndef _tlRoadSegmentChunk_HPP_
#define _tlRoadSegmentChunk_HPP_

#ifdef WIN32
#pragma warning(disable:4786)
#endif

#include "tlDataChunk.hpp"

// things included by tlRoadSegmentChunk.sc
#include <tlPoint.hpp>
#include <tlMatrix.hpp>
#include <srrchunks.h>

//class referred by tlRoadSegmentChunk.sc
class tlPoint;
class tlMatrix;
class tlRoadSegmentChunk;




class tlRoadSegmentChunk : public tlDataChunk 
{
public:

    // Methods
    tlRoadSegmentChunk(); 
    tlRoadSegmentChunk(tlReadChunk16* ch); 
    tlRoadSegmentChunk(const tlRoadSegmentChunk& ch); 
    ~tlRoadSegmentChunk();

    void Init();

    tlRoadSegmentChunk& operator=(const tlRoadSegmentChunk&); 
    virtual bool operator==(const tlDataChunk&);    // Returns true if the chunks are identical
    virtual bool NameCompare(const tlDataChunk&);   // Returns true if the chunks are the
                                                                    // same type and have the same name

    static tlDataChunk* Create(tlReadChunk16* ch);
    virtual long Size( );
    void   Write(tlFile* f);
    void   Print(int indent = 0,
                     unsigned int printdepth = 0xffffffff,
                     unsigned int typemask = 0xffffffff);
    void   PrintFormatted(int indent = 0);

    const char* GetType() const;
    int         GetFieldCount() const;
    bool        GetFieldIsArray(int) const;
    int         GetFieldArrayCount(int) const;
    const char* GetFieldType(int) const;
    const char* GetFieldName(int) const;
    bool        GetFieldValue(int, char*, int) const;
    bool        GetFieldArrayValue(int, int, char*, int) const;
    bool        GetFieldUpdatable(int) const;
    bool        SetFieldValue(int, const char*);
    bool        SetFieldArrayValue(int, int, const char*);

    static tlDataChunk* LoadSubChunk(tlFile* f);
    tlDataChunk* Copy(); 

    virtual int SortPriority() {return tlRoadSegmentChunk::sortPriority;}

    // Accessor Methods;

    const char* Name( );
    const char* GetName() { return Name(); }
    void     SetName(const char*);

    const char* RoadSegmentData( );
    const char* GetRoadSegmentData() { return RoadSegmentData(); }
    void     SetRoadSegmentData(const char*);

    tlMatrix HierarchyMatrix( );
    tlMatrix GetHierarchyMatrix() { return HierarchyMatrix(); }
    void     SetHierarchyMatrix(tlMatrix);

    tlMatrix ScaleMatrix( );
    tlMatrix GetScaleMatrix() { return ScaleMatrix(); }
    void     SetScaleMatrix(tlMatrix);

private:

    // Static data
    static int sortPriority;

    // Data Members
    char*    name;
    char*    roadsegmentdata;
    tlMatrix hierarchymatrix;
    tlMatrix scalematrix;

};

#endif

