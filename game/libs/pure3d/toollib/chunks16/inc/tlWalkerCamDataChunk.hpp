
// Automatically generated by Jeeves, don't edit.  Edit tlWalkerCamDataChunk.sc instead

#ifndef _tlWalkerCamDataChunk_HPP_
#define _tlWalkerCamDataChunk_HPP_

#ifdef WIN32
#pragma warning(disable:4786)
#endif

#include "tlDataChunk.hpp"

// things included by tlWalkerCamDataChunk.sc
#include <srrchunks.h>
#include <tlPoint.hpp>

//class referred by tlWalkerCamDataChunk.sc
class tlPoint;
class tlWalkerCamDataChunk;




class tlWalkerCamDataChunk : public tlDataChunk 
{
public:

    // Methods
    tlWalkerCamDataChunk(); 
    tlWalkerCamDataChunk(tlReadChunk16* ch); 
    tlWalkerCamDataChunk(const tlWalkerCamDataChunk& ch); 
    ~tlWalkerCamDataChunk();

    void Init();

    tlWalkerCamDataChunk& operator=(const tlWalkerCamDataChunk&); 
    virtual bool operator==(const tlDataChunk&);    // Returns true if the chunks are identical
    virtual bool NameCompare(const tlDataChunk&);   // Returns true if the chunks are the
                                                                    // same type and have the same name

    static tlDataChunk* Create(tlReadChunk16* ch);
    virtual long Size( );
    void   Write(tlFile* f);
    void   Print(int indent = 0,
                     unsigned int printdepth = 0xffffffff,
                     unsigned int typemask = 0xffffffff);
    void   PrintFormatted(int indent = 0);

    const char* GetType() const;
    int         GetFieldCount() const;
    bool        GetFieldIsArray(int) const;
    int         GetFieldArrayCount(int) const;
    const char* GetFieldType(int) const;
    const char* GetFieldName(int) const;
    bool        GetFieldValue(int, char*, int) const;
    bool        GetFieldArrayValue(int, int, char*, int) const;
    bool        GetFieldUpdatable(int) const;
    bool        SetFieldValue(int, const char*);
    bool        SetFieldArrayValue(int, int, const char*);

    static tlDataChunk* LoadSubChunk(tlFile* f);
    tlDataChunk* Copy(); 

    virtual int SortPriority() {return tlWalkerCamDataChunk::sortPriority;}

    // Accessor Methods;

    unsigned long ID( );
    unsigned long GetID() { return ID(); }
    void          SetID(unsigned long);

    float         MinMagnitude( );
    float         GetMinMagnitude() { return MinMagnitude(); }
    void          SetMinMagnitude(float);

    float         MaxMagnitude( );
    float         GetMaxMagnitude() { return MaxMagnitude(); }
    void          SetMaxMagnitude(float);

    float         Elevation( );
    float         GetElevation() { return Elevation(); }
    void          SetElevation(float);

    tlPoint       TargetOffset( );
    tlPoint       GetTargetOffset() { return TargetOffset(); }
    void          SetTargetOffset(tlPoint);

private:

    // Static data
    static int sortPriority;

    // Data Members
    unsigned long id;
    float         minmagnitude;
    float         maxmagnitude;
    float         elevation;
    tlPoint       targetoffset;

};

#endif

