...
#parameters:  command line
    ...
    # Only show output if there was no error (to avoid duplicate output)
    if ($ERRORLEVEL == $SUCCESS) {
        print GetTempLog();
    }

    # Comment out KillTempLog() to preserve error files for debugging
    # KillTempLog();
    print "\n";
